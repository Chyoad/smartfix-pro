...
async function initDatabase() {
  ...
  // Column Migrations
  try { await db.exec("ALTER TABLE tickets ADD COLUMN repairHistory TEXT"); } catch (e) {}
  try { await db.exec("ALTER TABLE tickets ADD COLUMN warrantyDuration INTEGER"); } catch (e) {}
  try { await db.exec("ALTER TABLE tickets ADD COLUMN isWarrantyClaim INTEGER DEFAULT 0"); } catch (e) {}
  try { await db.exec("ALTER TABLE tickets ADD COLUMN originalTicketId VARCHAR(255)"); } catch (e) {}
  ...
  try {
    ...
    if (!adminCount || adminCount.count === 0) {
      await db.run(
        ...
        ["admin-root", "admin@service.com", "Administrator", "admin"]
      );
    }
  } catch (err) {
...
async function startServer() {
  ...
  app.post("/api/users", async (req, res) => {
    try {
      ...
      await db.run("INSERT INTO users (uid, email, displayName, role, isActive) VALUES (?, ?, ?, ?, ?)", [
        id, sanitize(email), sanitize(displayName), sanitize(role), isActive ? 1 : 0
      ]);
  ...
  app.put("/api/inventory/:id", async (req, res) => {
    try {
      ...
      await db.run(`
        ...
        WHERE id = ?
      `.replace("text = stockLevel,", ""), [ // Quick fix/cleanup
        updated.name, updated.sku, updated.barcode, updated.category,
        updated.stockLevel, updated.minStockAlert, updated.buyPrice,
        updated.sellPrice, updated.warehouse, id
      ]);
  ...
  app.post("/api/tickets", async (req, res) => {
    try {
      ...
      await db.run(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `, [
        id, sanitize(data.customerId), sanitize(data.customerName), sanitize(data.deviceModel), sanitize(data.imei), sanitize(data.problem), sanitize(data.diagnosis), sanitize(data.status),
        sanitize(data.technicianId), sanitize(data.technicianName), sanitize(data.estimatedCost), sanitize(data.finalCost), 
        JSON.stringify(data.partsUsed || []), JSON.stringify(data.conditionPhotos || []), JSON.stringify(data.resultPhotos || []), sanitize(data.warrantyExpiry), sanitize(data.warrantyDuration),
        data.isWarrantyClaim ? 1 : 0, sanitize(data.originalTicketId),
    ...
    try {
      ...
      const result = await db.transaction(async (scopedDb) => {
        ...
        await scopedDb.run(`
          ...
          WHERE id = ?
        `, [
          updated.status, updated.diagnosis, updated.technicianId, updated.technicianName,
          updated.estimatedCost, updated.finalCost, JSON.stringify(updated.partsUsed || []), updated.warrantyExpiry, 
          updated.warrantyDuration, updated.isWarrantyClaim ? 1 : 0, sanitize(updated.originalTicketId), 
          updated.updatedAt, JSON.stringify(updated.resultPhotos || []),
          JSON.stringify(updated.repairHistory || []),
          id
        ]);
        return updated;
      });
  ...
  app.post("/api/transactions", async (req, res) => {
    try {
      ...
      await db.run(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `, [
        id, sanitize(data.type), sanitize(data.description), sanitize(data.amount), sanitize(data.totalAmount), sanitize(data.category), sanitize(data.paymentMethod),
        JSON.stringify(data.items || []), sanitize(data.ticketId)
      ]);
      res.json({ id, ...data });
    } catch (e: any) {
      console.error(e);
