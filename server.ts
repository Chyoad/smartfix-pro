...
if (process.env.VERCEL) {
  // On Vercel, initialize the database on cold start without starting a listening server
  initDatabase().catch((err) => console.error('Immediate Vercel DB init failed:', err))
} else {
  // On standard/local environments, start the server and run migrations
  initDatabase()
    .then(() => startServer())
    .catch((err) => {
      console.error('Database init failed:', err)
      // Still start server to serve the frontend if DB fails initially
      startServer()
    })
}
